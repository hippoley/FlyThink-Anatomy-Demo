...
def make_dialogues(split,count,seed):
 ...
 for i in range(count):
  ...
  for turn in range(10):
   ...
   if active:choices+=['revise']*2+['retract','pause','resume']+(['retract_named','revise_named'] if named else [])
   ...
   if op=='add':
    ...
    k=state['focus'] if state['focus']<NONE else rng.choice(active);value=rng.choice([1,2] if k%2==0 else [3,4])
    ...
    k=rng.choice(named);room=k//4;obj=(k%4)//2;value=rng.choice([1,2] if k%2==0 else [3,4])
    ...
    k=state['focus'] if state['focus']<NONE else rng.choice(active)

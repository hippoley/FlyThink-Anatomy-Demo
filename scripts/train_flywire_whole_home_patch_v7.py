...
from whole_home_patch_corpus_v6 import build
...
def main():
 a=argparse.ArgumentParser();a.add_argument("--graph",default="artifacts/flywire/connectome.json");a.add_argument("--epochs",type=int,default=240);a.add_argument("--out",type=Path,default=Path("artifacts/flywire-whole-home-v6"));q=a.parse_args()
 ...
 rep={"truth":"v7_deployment_aware_relative_binding","source_sha256":EXPECTED_SHA256,"graph_sha256":GRAPH_SHA,"train_examples":len(d["train"]),"dev_examples":len(d["dev"]),"final_test_examples":len(d["sealed"]),"runs":{}}
 ...
 rep["real_minus_disconnected_exact"]=rep["runs"]["real"]["final_test"]["exact"]-rep["runs"]["disconnected"]["final_test"]["exact"];rep["passes_90_gate"]=rep["runs"]["real"]["final_test"]["exact"]>=0.90;(q.out/"report.json").write_text(json.dumps(rep,ensure_ascii=False,indent=2));print(json.dumps(rep,ensure_ascii=False))

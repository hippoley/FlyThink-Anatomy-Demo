...
def build():
    ...
    # Relative contrastive expansion: direction must generalize independently of slot/device.
    relative_train=[
      ("温度低一点",AC_L,"temperature",-1),("温度高一点",AC_L,"temperature",1),
      ("卧室这个降一点",AC_B,"temperature",-1),("卧室这个升一点",AC_B,"temperature",1),
      ("窗户开小一点",WIN_L,"opening",-1),("窗户开大一点",WIN_L,"opening",1),
      ("再关小一些",WIN_L,"opening",-1),("再开大一些",WIN_L,"opening",1),
      ("往下降一点",AC_L,"temperature",-1),("往上升一点",AC_L,"temperature",1),
      ("少一点",AC_L,"temperature",-1),("多一点",AC_L,"temperature",1),
    ]
    for t,x,s,dlt in relative_train:
      tr.append(r(t,"PATCH_RELATIVE",x,s,delta=dlt,lifecycle={"focused_target":x},family="relative"))
    ...
    # dev: checkpoint selection only; final test below is never used for selection.
    dev += [
      r("卧室那个也开上","ADD_DEVICE",AC_B,"power","ON",lifecycle={"focused_target":AC_L},family="additive"),
      r("它关一下","CLOSE_DEVICE",AC_B,"power","OFF",lifecycle={"focused_target":AC_B},family="focus"),
      r("刚那条算了","CANCEL_PENDING",lifecycle={"pending_ids":["pd"]},family="cancel"),
      r("刚那条算了","UNDO_EXECUTED",lifecycle={"executed_ids":["ed"]},family="undo"),
      r("这个稍微降一点","PATCH_RELATIVE",AC_L,"temperature",delta=-1,lifecycle={"focused_target":AC_L},family="relative"),
      r("这个稍微升一点","PATCH_RELATIVE",AC_L,"temperature",delta=1,lifecycle={"focused_target":AC_L},family="relative"),
      r("窗户再小一点","PATCH_RELATIVE",WIN_L,"opening",delta=-1,lifecycle={"focused_target":WIN_L},family="relative"),
      r("窗户再大一点","PATCH_RELATIVE",WIN_L,"opening",delta=1,lifecycle={"focused_target":WIN_L},family="relative"),
      r("这两个都开","PATCH_SLOT",slot="power",value="ON",targets=[AC_L,AC_B],lifecycle={"referent_set":[AC_L,AC_B]},family="set"),
    ]
    # final test: unseen surfaces + context counterfactuals; never checkpoint-selected.
    se += [
      ...
      r("开度收一点","PATCH_RELATIVE",WIN_L,"opening",delta=-1,lifecycle={"focused_target":WIN_L},family="relative"),
      r("开度放大一点","PATCH_RELATIVE",WIN_L,"opening",delta=1,lifecycle={"focused_target":WIN_L},family="relative"),
      r("这俩都打开","PATCH_SLOT",slot="power",value="ON",targets=[AC_L,AC_B],lifecycle={"referent_set":[AC_L,AC_B]},family="set"),
    ]
    return {"truth":"v6_contrastive_relative_generalization","train":tr,"dev":dev,"sealed":se,
            "requirements":["minimal_patch","set_cardinality","relative_not_absolute","context_counterfactual","untouched_state_100_percent"]}
if __name__=="__main__":
 import json;d=build();print(json.dumps({"train":len(d["train"]),"dev":len(d["dev"]),"final_test":len(d["sealed"]),"truth":d["truth"]},ensure_ascii=False))

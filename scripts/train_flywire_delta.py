...
def resolve(pred,examples):
 ...
 for i,e in enumerate(examples):
  ...
  if ref==REFS.index('focus_coreference') and before['focus']<NONE:
   k=before['focus'];pred[i,3]=k//4;pred[i,4]=(k%4)//2;pred[i,5]=k%2
   ...
   if r<3 and o<2:
    ...
    if len(active)==1:pred[i,5]=active[0]%2
 return pred
...
def main():
 ...
 p.add_argument('--modes',nargs='+',default=['real','frozen','rewired','disconnected']);p.add_argument('--out',type=Path,default=Path('artifacts/flywire-delta'));a=p.parse_args()
 ...
 data=corpus();train,test,sealed=[pack(data[k]) for k in ['train','test','sealed']];a.out.mkdir(parents=True,exist_ok=True)
 ...
 for mode in a.modes:
  ...
  for epoch in range(a.epochs):
   ...
   if (epoch+1)%10==0:print(json.dumps({'mode':mode,'epoch':epoch+1,'loss':logs[-1]}),flush=True)

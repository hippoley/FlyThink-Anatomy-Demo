...
from train_flywire_whole_home_patch import context_features, encode_patch, objective, predict, metrics, MAX_PATCHES
...
def train_mode(g,train,sealed,mode,epochs):
    model=FlyWireGatedPatchNet(g,disconnect=(mode=="disconnected"))
...
def main():
    ...
    report={"truth":"v3_flywire_gated_context_counterfactual_training","source_sha256":EXPECTED_SHA256,"graph_sha256":GRAPH_SHA,
    ...
    report["interpretation"]="connectome advantage observed" if r>d else "no demonstrated connectome advantage on v3 gated architecture"
